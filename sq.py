...
parser = argparse.ArgumentParser(
    description="Shockley-Queisser calcs for an ideal solar cell (n=1, no parasitic resistances)",
    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
)

parser.add_argument(
    "--t-cell", default=25, type=np.double, help="Temperature of the solar cell [deg C]"
)
parser.add_argument(
    "--band-gap",
    default=1.35,
    type=np.double,
    help="Band gap of the solar cell [eV] (ignored if --device-absorption-file is given)",
)
parser.add_argument(
    "--no-plot", default=False, action="store_true", help="Disable plot"
)
parser.add_argument(
    "--solar-spectra-file",
    default="ASTMG173.csv",
    help="File to read the solar spectra from",
)
parser.add_argument(
    "--device-absorption-file",
    default="",
    help="File to read the absorption spectrum from",
)
...
with open(args.solar_spectra_file) as sunfile:
    ...
    for row in stream:
        if len(row) == 2:
            ...
            etr = np.append(
                etr, np.double(row[1])
            )  # [W/m^2/nm] extra terrestrial radiation
            am15 = np.append(
                am15, np.double(row[2])
            )  # [W/m^2/nm] "Global Tilt" = spectral radiation from solar disk plus sky diffuse and diffuse reflected from ground on south facing surface tilted 37 deg from horizontal
...
def current(E_cutoff):
    ...
    if not all(bool_vec):  # we're some place in the middle
        these_fluxes = np.append(
            these_fluxes, np.interp(E_cutoff, E_solar, fluxes, left=0, right=0)
        )
...
print("")
print(
    f"and if we shine light as defined by the file {Path(args.solar_spectra_file).name} at it, then"
)
print("")
...
if args.no_plot == False:
    ...
    if daf.is_file():
        print(
            f"The Current Through A {area_str}Semi-Perfect Solar Cell at",
            T_cell - K_offset,
            "deg C",
            file=buffer,
            end="",
        ...
        print(
            f"The Current Through A {area_str}Perfect,",
            E_BG / q,
            "eV Solar Cell at",
            T_cell - K_offset,
            "deg C",
